...
// single instance of the Config structure
var instance = &Config{
	data:           make(map[string]string),
	watchInterval:  time.Second * 10,
	stop:           make(chan bool),
	toWatchLoaders: make([]Loader, 0),
}

//Get Instantiate new config reader
func Get() *Config {
	return instance
}

func Load(loaders ...Loader) (*Config, error) {
	return instance, instance.Load(loaders...)
}
...
// StopWatch stopping all watches
...
func (c *Config) Load(loaders ...Loader) error {
	...
	for _, loader := range loaders {
		...
		if loader.IsWatchable() {
			c.toWatchLoaders = append(c.toWatchLoaders, loader)
		}
	}
...
func (c *Config) GetFloat(key string) (float64, bool) {
	...
	intVal, err := strconv.ParseFloat(c.resolveVar(val), 64)
	if err != nil {
		return 0, false
	}
	return intVal, true
...
func (c *Config) GetDuration(key string) (time.Duration, bool) {
	...
	duration, err := ParseDuration(c.resolveVar(val))
	if err != nil {
		return 0, false
	}
...
func (c *Config) resolveVar(val string) string {
...
// GetKeys return all loaded keys
...
func (c *Config) GetBool(key string) (bool, bool) {
	...
	boolVal, ok := truthTable[strings.ToLower(c.resolveVar(val))]
	if !ok {
		return false, false
	}
	return boolVal, true
...
func (c *Config) Unmarshal(destinationPtr interface{}) error {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		c.data[defKey] = defValue

		fv := indirect(pv.Field(i), false)

		if fv.CanSet() {
			var v reflect.Value
			if fv.Type().AssignableTo(reflect.TypeOf(time.Duration(0))) {
				fv.Set(decode(reflect.ValueOf(c.GetDuration), cfgKey, defValue))
			} else {
				switch fv.Kind() {
				case reflect.Bool:
					v = decode(reflect.ValueOf(c.GetBool), cfgKey, defValue)
				case reflect.String:
					v = decode(reflect.ValueOf(c.GetString), cfgKey, defValue)
				case reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64,
					reflect.Uint, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64, reflect.Int:
					v = decode(reflect.ValueOf(c.GetInt), cfgKey, defValue).Convert(fv.Type())
				case reflect.Float32, reflect.Float64:
					v = decode(reflect.ValueOf(c.GetFloat), cfgKey, defValue).Convert(fv.Type())
				case reflect.Struct:
					_ = c.Unmarshal(fv.Addr().Interface())
				}

				if v.IsValid() {
					fv.Set(v)
				} else if !v.IsValid() {
					err = fmt.Errorf("%s not set value and don't have default value", cfgKey)
				}
			}
		}
	}
...
func (c *Config) watch() {
	timer := time.NewTimer(c.watchInterval)
	for {
		select {
		case <-timer.C:
			c.lock.Lock()
			for _, loader := range c.toWatchLoaders {
				values, err := loader.Parse()
				if err != nil {
					fmt.Println(err)
					continue
